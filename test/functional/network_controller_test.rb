...
class NetworkControllerTest < ActionController::TestCase
  ...
  def test_show_should_default_to_first_page_of_my_network
    login_as(:quentin)
    get :show
  ...
  def DISABLED_FOR_ALPHA_test_show_partial_should_display_network_link
    login_as(:quentin)
    get :show
    assert_response :success
    assert_template "show"
    ...
    assert_no_match /Pick Consultant/, @response.body
  end
end
