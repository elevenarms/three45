#
# (Generated from the RESTful Authentication plugin)
#
# Provides helper methods for accessing the currently logged in user, along with a before filter,
# login_required, for enforcing a user login prior to action processing.
#
#
#
     ...
     def access_denied
          respond_to do |format|
            ...
            format.html do
              ...
              subd
              @wg = Workgroup.find(:first, :conditions => {:subdomain=>@subdomain})
              unless @wg.nil?
                if @wg.subscriber?
                  store_location
                  ...
                  store_location
                  redirect_to :controller => '/uns_welcome', :action => 'new'
                end
              else
                render :file => "#{RAILS_ROOT}/public/noaccount.html" 
              end
            end
            format.xml do
